	...
	private void addActionToEncryptButton() {
		encryption.setOnAction(event -> {
			try {
				String bellasoStr = "";
				String encryptedStr = "";
				String rawInput = plainTextTextField.getText();

				if (!CryptoManager.isStringInBounds(rawInput)) {
					encryptedStringTextField3.setText("The selected string is not in bounds. Try again.");
					return;
				}

				String plainText = rawInput.toUpperCase();

				if (radioButton1.isSelected()) {
					shiftInt = Integer.parseInt(inputForEncryptionTextField.getText());
					encryptedStr = CryptoManager.caesarEncryption(plainText, shiftInt);
				} else {
					bellasoStr = inputForEncryptionTextField.getText().toUpperCase();
					inputForEncryptionTextField.setText(bellasoStr);
					encryptedStr = CryptoManager.bellasoEncryption(plainText, bellasoStr);
				}
